...
TrafficLight::TrafficLight(const carla_msgs::CarlaTrafficLightInfo &traffic_light_info,
  ...
  const auto traffic_light_center = common::MathUtils::Transform(transform_, center_point);
...
//  center_xy << traffic_light_center.x(), traffic_light_center.y();
  ...
  traffic_light_box_ = common::Box2d(center_xy, light_yaw, trigger_volume_.size.x, trigger_volume_.size.y);
}
}