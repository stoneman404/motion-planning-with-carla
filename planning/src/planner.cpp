...
Planner::Planner(const ros::NodeHandle &nh) : nh_(nh), thread_pool_size_(8) {
  ...
  this->behaviour_planner_ = std::make_unique<BehaviourPlanner>(nh_,"mpdm", thread_pool_.get());
  this->InitPublisher();
  this->InitSubscriber();
...
void Planner::InitServiceClient() {
  ...
  this->route_service_client_ = nh_.serviceClient<planning_srvs::RoutePlanService>(
      common::service::kRouteServiceName);
...
bool Planner::GetWayPoint(const int &object_id,
  ...
  if (!get_actor_waypoint_client_.call(srv)) {
    ROS_FATAL("Cannot Get actor's waypoint");
    return false;
  }
...
void Planner::VisualizeRoute(std::vector<planning_srvs::RoutePlanServiceResponse> &routes) {
  visualization_msgs::MarkerArray marker_array;
  int i = 0;
