...
class RandomRoutePlanner(object):
    """
    ...
    def _find_loose_ends(self):
        ...
        for segment in self._topology:
            ...
            if road_id in self._road_id_to_edge and section_id in self._road_id_to_edge[road_id] and lane_id in \
    ...
    def _turn_decision(self, index, route, threshold=math.radians(35)):
        ...
        if index > 0:
            if self._previous_decision != RoadOption.VOID and self._intersection_end_node > 0 and self._intersection_end_node != previous_node and \
                    next_edge['type'] == RoadOption.LANEFOLLOW and next_edge['intersection']:
    ...
    def get_potential_paths(self, location):
        edge = self._localize(location)
        graph_edge = self._graph.edges[edge[0], edge[1]]
        path = graph_edge['path']
        # if path:
        #     # for wp in path:
        #         # draw_waypoints(world, path, z=0.5)
        print(edge[0], edge[1])
        route = []
        for neighbor, w in self._graph[edge[1]].items():
            print(w['intersection'])
            if len(w['path']) < 2:
                continue
            next_path = path + w['path'] + [w['exit_waypoint']]
            route.append(next_path)
        return route
...
if __name__ == '__main__':
    ...
    m = world.get_map()
    transform_list = m.get_spawn_points()
    locations = []
    for transform in transform_list:
        locations.append(transform.location)
    w = m.get_waypoint(random.choice(locations))
    left_w = None
    right_w = None
    if w.lane_change == carla.LaneChange.Left or w.lane_change == carla.LaneChange.Both:
        left_w = w.get_left_lane()

    if w.lane_change == carla.LaneChange.Right or w.lane_change == carla.LaneChange.Both:
        right_w = w.get_right_lane()

    world.debug.draw_point(w.transform.location + carla.Location(z=1), color=Color(100, 255, 1), life_time=120)
    clock = pygame.time.Clock()
    dao = GlobalRoutePlannerDAO(
        m, sampling_resolution=2.0)
    random_planner = RandomRoutePlanner(dao)
    random_planner.setup()
    path = random_planner.trace_route(w.transform.location, carla.Location(x=300.0, y=-100.0, z=2))
    for pp in path:
        world.debug.draw_point(pp[0].transform.location, life_time=120)

    paths = random_planner.get_potential_paths(w.transform.location)
    if left_w: left_path = random_planner.get_potential_paths(left_w.transform.location)
    if right_w: right_path = random_planner.get_potential_paths(right_w.transform.location)
